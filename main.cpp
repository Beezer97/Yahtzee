...
int nummove(0);	// Переменная для очередности ходов, глобальная, потому что используется в pointscount, а обнуляется в game
...
{
	int select(0);
	while(select != 49) // Пока не будет нажата 1(начать игру), menu будет выполняться
	{
		system("cls");
...
int countcomb(int dice[5]) // Функция распознавания комбинаций
{
	int points(1); // Ничего
	//Покер
...
{
	bool used[5] = { false,false,false,false,false }; // Массив, который будет содержать данные перекинутых костей, чтобы во второй раз перекидывались только они
	...
	for (int i = 0; i < 5; i++)
		dice[i] = RANDOM;
	for (int i = 0; i < 5; i++) // Копируем массив для сортировки(нужно для определения комбинации), если будем сортировать начальный - будет выводится отсортированный
		sortdice[i] = dice[i];
...
int pointscount(string player1, string player2)
{
	...
	int sortdice1[5] = { 0,0,0,0,0 }, sortdice2[5] = { 0,0,0,0,0 }; // Создаем массивы для проверки комбинации у игроков(dice1 и dice2 нам нужны неотсортированные для вывода)
	for (int i = 0; i < 5; i++)
	{
		sortdice1[i] = dice1[i];
		sortdice2[i] = dice2[i];
	...
	points1 = countcomb(sortdice1);
	points2 = countcomb(sortdice2);
...
{
	int sysgame(0), res(0), sc1(0), sc2(0);
	system("cls");
...
int main()
{
	...
	int anew(0);
	do // Работает пока пользователь не отказывается от возврата в меню
	{
		menu(); // Меню
		for (;;)
		{
			...
			if (player1 == player2)
			{
				cout << "\nИмена игроков не могут быть одинаковыми, попробуйте еще раз:\n" << endl;
				continue;
			}
			break;
		}
